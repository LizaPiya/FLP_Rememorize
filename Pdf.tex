...
All models are prompted using a source-grounded instruction adapted from~\citet{asgari2025framework}, which explicitly constrains generation to information present in the input document. The full prompt template is provided in Appendix~\ref{appendix:annotation_prompt}.
...
\textsc{ReMemorize} achieves the highest faithfulness score on MIMIC-IV (63.48\%) and the lowest hallucination rate on SOAP (5.91\%), outperforming all evaluated baselines on factual grounding. MedAlpaca-7B achieves the highest ROUGE-L on MIMIC (13.70), while \textsc{ReMemorize} leads on faithfulness by a substantial margin (+9.59 over the next best). Flan-T5 XXL lags on both ROUGE and faithfulness despite its scale, reflecting the difficulty of zero-shot clinical summarization without domain adaptation.

\begin{table*}[t]
\centering
\caption{Summary generation performance on MIMIC-IV-Ext-BHC and SOAP datasets.
MIMIC evaluated on $n{=}500$ test samples; SOAP on $n{=}300$ test samples.
All values in \%. Higher is better for R-1, R-2, R-L, BS-F1, and Factuality ($\uparrow$); lower is better for Hallucination ($\downarrow$).
Factuality scored via MiniCheck (NLI entailment probability); Hallucination via NLI contradiction probability.
Values reported as mean $\pm$ bootstrap std (1000 iterations).}
\label{tab:main_results}
\begin{tabular}{lrrrrrrrrrrrr}
\toprule
& \multicolumn{6}{c}{\textbf{MIMIC-IV-Ext-BHC}} & \multicolumn{6}{c}{\textbf{SOAP}} \\
\cmidrule(lr){2-7} \cmidrule(lr){8-13}
\textbf{Model}
& \textbf{R-1} & \textbf{R-2} & \textbf{R-L} & \textbf{BS-F1} & \textbf{Fact.}$\uparrow$ & \textbf{Hall.}$\downarrow$
& \textbf{R-1} & \textbf{R-2} & \textbf{R-L} & \textbf{BS-F1} & \textbf{Fact.}$\uparrow$ & \textbf{Hall.}$\downarrow$ \\
\midrule
...
\begin{table*}[t]
\centering
\caption{LLM-as-a-Judge evaluation (Qwen2.5-7B-Instruct, 0\% parse failure rate).
MIMIC evaluated on $n{=}500$; SOAP on $n{=}300$. All scores on a 1--5 scale.
Higher is better except Hallucination ($\downarrow$). Values reported as mean $\pm$ bootstrap std.}
\label{tab:llm_judge}
...
\begin{table}[t]
\centering
\caption{Ablations on MIMIC-IV ($n{=}500$). All values in \%. Bold = best per metric.
Fact./Hall.\ = MiniCheck faithfulness / NLI contradiction probability.
Judge = Qwen2.5-7B-Instruct scores (1--5 scale).
Values reported as mean $\pm$ bootstrap std (1000 iterations).}
...
\section{Hallucination Annotation Prompt}
\label{appendix:annotation_prompt}

To identify hallucinated spans in model-generated clinical summaries, we employ a structured evaluation prompt grounded in strict textual entailment. The task requires a language model to verify whether each statement in a generated summary is \textbf{verifiably supported} by the source document---meaning the information must be \textit{explicitly stated and unambiguous} in the document text.
This prompt is adapted from the Lookback Lens~\citep{chuang2024lookback} framework, but incorporates additional guardrails specific to clinical applications. Crucially, it includes explicit instructions to prevent confirmation bias~\citep{adiba_bias_2025}, discourage plausible-sounding inferences, and eliminate reliance on domain knowledge not grounded in the text.

\subsubsection*{Prompt Template}
\begin{quote}\small
You will be provided with a clinical source document and a machine-generated summary. Your task is to assess whether the summary is \textbf{strictly entailed} by the source document.

\medskip
\textbf{Definition of Entailment:} A statement is entailed only if it is explicitly and unambiguously stated in the document. Any statement that requires inference, clinical reasoning, or prior medical knowledge should be treated as \textit{Not Entailed}.

\medskip
\textbf{Key Instructions:}
\begin{itemize}[noitemsep]
  \item DO NOT infer or assume missing information.
  \item DO NOT use clinical knowledge, reasoning, or common sense.
  \item DO NOT validate based on plausibility, typicality, or tone.
  \item DO NOT guess what the author likely meant.
\end{itemize}
You must treat the document as the \textbf{only source of ground truth}.

\medskip
\textbf{Document:} \{document\} \quad \textbf{Proposed Summary:} \{summary\}

\medskip
\textbf{Output Format:}
\begin{itemize}[noitemsep]
  \item \textbf{Entailment Label:} \texttt{Entailed} or \texttt{Not Entailed}
  \item \textbf{Explanation:} Justify your label with direct references to the document.
  \item \textbf{Problematic Spans (if any):} List summary phrases not directly supported by the source.
\end{itemize}
\end{quote}
...
The following prompt is used to evaluate generated clinical summaries using an instruction-tuned LLM acting in an evaluative role. The model is instructed to assess summaries strictly with respect to the provided source document, without relying on external medical knowledge or plausible inference. This protocol follows our prior work~\citep{asgari2025framework}.

\begin{quote}\small
\textbf{Task:} Evaluate the generated medical summary against the source clinical document. Assign an integer score from 1 to 5 for each criterion below.

\medskip
\textbf{Source Document:} \{SOURCE\_DOCUMENT\} \quad \textbf{Generated Summary:} \{GENERATED\_SUMMARY\}

\medskip
\textbf{Evaluation Criteria (1--5 scale):}
\begin{itemize}[noitemsep]
    \item \textbf{Hallucination:} Degree of unsupported or fabricated content (1\,=\,no hallucination; 5\,=\,major fabrications)
    \item \textbf{Factual Consistency:} Faithfulness to the source (1\,=\,highly inaccurate; 5\,=\,fully accurate)
    \item \textbf{Completeness:} Coverage of core clinical information (1\,=\,key info missing; 5\,=\,fully comprehensive)
    \item \textbf{Coherence:} Fluency and logical organisation (1\,=\,poorly written; 5\,=\,highly coherent)
\end{itemize}

\medskip
\textbf{Output Format} (no additional text):
\begin{verbatim}
Hallucination: X
Factual: X
Complete: X
...
Evaluation uses a decoding temperature of 0.3 and a maximum output length of 150 tokens. All scores are validated to lie within the 1--5 range, yielding a reproducible and fully automated assessment protocol.
